...
//    Employee[] employees=new Employee[10];
    ...
    //删除一个员工   或者删除一个部门
    public void deleteEmployee(Employee employee, Company company) {
        Department[] department = company.getDepartment();  //获取部门列表
        for (int i = 0; i < company.getDeptNumber(); i++) {
            if (department[i].getDeptName().equals(employee.getDepartment().getDeptName())) {
                //创建人事部的员工数组
                Employee[] employee1 = department[i].getEmployee();
                Employee[] newEmp = new Employee[employee1.length - 1];
                int key = -1;
                for (int j = 0; j < employee1.length; j++) {
                    if(employee1[i].getName() == employee.getName()){
                        key = i;
                    }else{
                        return;
                    }
                ...
                for (int j = 0; j < newEmp.length; j++) {
                    if(i<key){
                        newEmp[i] = employee1[i];
                    }else{
                        newEmp[i] = employee1[i+1];
                    }
                }
                department[i].setEmployee(newEmp);
            }
        }
    }
